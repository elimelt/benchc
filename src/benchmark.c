...
void bench_init(void) {
    ...
    char *env;
    if ((env = getenv("BENCH_ITERS"))) config.iterations = (uint64_t)atol(env);
    if ((env = getenv("BENCH_WARMUP"))) config.warmup_iterations = (uint64_t)atol(env);
    if ((env = getenv("BENCH_CSV"))) config.output_file = env;
    if ((env = getenv("BENCH_QUIET"))) config.verbose = !atoi(env);
    bench_init_config(&config);
}

void bench_init_config(const bench_config_t *config) {
    g_bench.result_count = 0;
    if (config) g_bench.config = *config;
    g_bench.initialized = 1;
}

void bench_register(bench_fn_t fn, const char *name, const char *description) {
    if (g_bench.count >= BENCH_MAX_BENCHMARKS) return;
...
static int run_single_benchmark(bench_entry_t *entry, bench_result_t *result) {
    ...
    for (uint64_t i = 0; i < warmup; i++) entry->fn();
...
int bench_run_all(void) {
    if (!g_bench.initialized) bench_init();
    if (g_bench.config.verbose) printf("=== Running %zu benchmarks ===\n\n", g_bench.count);
    g_bench.result_count = 0;
    ...
    if (g_bench.config.output_file) bench_write_csv(g_bench.config.output_file);
    return 0;
}
...
const bench_result_t *bench_get_results(size_t *count) {
    if (count) *count = g_bench.result_count;
...
int bench_write_csv(const char *filename) {
    ...
    fclose(fp);
    if (g_bench.config.verbose) printf("Results written to %s\n", filename);
    return 0;
}
